...
suite('Unit Tests', function () {
  ...
  suite('Equality', function () {
    ...
    test('#equal, #notEqual', function () {
      ...
      assert.notEqual({ value: 1 }, { value: 1 }, '== compares object references'); // object will compared by reference by value 
      assert.equal(6 * '2', '12');
      assert.notEqual(6 + '2', '12'); // actual result is 62
    });
    // #6
    test('#strictEqual, #notStrictEqual', function () {
      ...
      assert.notStrictEqual([1, 'a', {}], [1, 'a', {}]); // 배열, 객체는 참조값으로 비교되기 떄문에 값이 달라진다. 
    });
    // #7
  ...
  suite('Comparisons', function () {
    ...
    test('#isAbove, #isAtMost', function () {
      ...
      assert.isAtMost(1 - Math.random(), 1); // Math.rondom(): 0 ~ 1 사이의 임의의 값을 반환합니다.
    });
    // #9
    ...
    test('#approximately', function () {
      assert.approximately(weirdNumbers(0.5), 1, 0.5); // 0.51 ~ 1.49 
      assert.approximately(weirdNumbers(0.2), 1, 0.8); // 0.21 ~ 1.19
    });
  });
